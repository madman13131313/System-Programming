...
void storeVoltage(void) {
    if (bufferSize == 0) // allocate memory for databank
    {
		bufferStart = malloc(100 * sizeof(uint16_t));
		bufferSize = 100;
    }
	if (bufferIndex >= bufferSize) // check if there is space left in buffer
	{
		return;
	}
	*(bufferStart + bufferIndex) = lastCaptured; // store valtage value at current buffer index
